...
new_template = Template(
    name=p_name,
    jinja="{% if question == \"cause\" %} Effect: {% else %} Cause: {% endif %}{{ premise }} ||| {% if label != -1 %}{{ answer_choices[label] }}{% endif %}",
    reference="gyx",
    ...
    answer_choices="{% if question == \"cause\" %} Cause: {% else %} Effect: {% endif %}{{choice1}} ||| {% if question == \"cause\" %} Cause: {% else %} Effect: {% endif %}{{choice2}}"
)
