...
def get_model(args, device):
    model = OPTForCausalLM.from_pretrained(args.model_config)
...
def evaluate(args, tokenizer, model, dataset: ICLEvalDataset, epoch, device):
    ...
    with torch.no_grad():
        for it, (model_batch, no_model_batch) in enumerate(tqdm(dataloader, desc=f"Evaluating {dataset.data_name} {dataset.prompt_name}", disable=(dist.get_rank() != 0))):            
            ...
            # torch.save((preds, min_loss, gold_loss, option_losses), os.path.join(args.save, "res.pt"))
...
def evaluate_gen(args, tokenizer: GPT2Tokenizer, model: OPTForCausalLM, dataset: ICLEvalDataset, epoch, device):
