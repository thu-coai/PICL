...
import bmtrain.distributed as dist_bm
from bmtrain import synchronize
...
def icl_inner_train(args, tokenizer, model: GPT2ICL, icl_batch, outer_bs, device):
    dataset = ICLPtInnerTrainDataset(args, tokenizer, icl_batch)
    dataloader = DataLoader(
        dataset, batch_size=args.icl_inner_batch_size, shuffle=False, num_workers=args.num_workers, drop_last=False, collate_fn=dataset.collate)
    new_pkv = torch.zeros(
        ...
        model.config.dim_head,
        dtype=model.dtype,
        device=device)

    with torch.no_grad():
        for model_batch, no_model_batch in tqdm(dataloader, desc="Inner training", disable=True):
            ...
            outputs = model(**model_batch, use_cache=True, return_dict=True)
            pkv = outputs.past_key_values
            for i in range(pkv.size(2)):
                b, s = no_model_batch["pos"][i]
                l = no_model_batch["lengths"][i]
                x = pkv[:, :, i, :, :, :]
                new_pkv[:, :, b, :, s:s+l, :] = x[:, :, :, :l, :]
...
def forward(args, tokenizer, model: GPT2ICL, dataset: ManyBagICLTrainDataset, model_batch, no_model_batch, icl_batch, device, share_pkv=False, pkv=None):
    # all_pkv = icl_inner_train_1(args, tokenizer, model, icl_batch, device)
    # model_batch, no_model_batch = dataset.form_batch(model_batch, no_model_batch, icl_batch, all_pkv, model.dtype, device)
    if pkv is None:
        bs = 1 if share_pkv else len(model_batch["input_ids"])
        pkv = icl_inner_train(args, tokenizer, model, icl_batch, bs, device)
    if share_pkv:
    ...
    # torch.save(icl_batch, "icl_batch.pt")
    
    outputs = model(**model_batch, return_dict=True, output_logits=True)

    # torch.save(model_batch, "model_batch.pt")
...
def finetune(args, tokenizer: GPT2Tokenizer, model: GPT2ICL, optimizer: AdamOptimizer, lr_scheduler, dataset, device):
    ...
    train_dataloader = DistributedDataLoader(
        dataset['train'], batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, collate_fn=dataset["train"].collate)
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        for it, (model_batch, no_model_batch, icl_batch) in enumerate(train_dataloader):
            dataset["train"].move_to_device(model_batch, no_model_batch, device)

            torch.cuda.synchronize()
            st_time = time.time()

            outputs = forward(args, tokenizer, model, dataset["train"], model_batch, no_model_batch, icl_batch, device)
...
def evaluate(args, tokenizer, model, dataset: ManyBagICLEvalDataset, split, epoch, device):
    dataloader = DistributedDataLoader(
        dataset, batch_size=args.eval_batch_size, shuffle=False, num_workers=args.num_workers, collate_fn=dataset.collate)
    ...
    with torch.no_grad():
        pkv = None
        for it, (model_batch, no_model_batch, icl_batch) in enumerate(tqdm(dataloader, desc=f"Evaluating {dataset.data_name} {dataset.prompt_name}", disable=(bmt.rank() != 0))):
            dataset.move_to_device(model_batch, no_model_batch, device)

            if split == "test" and args.icl_share_demo:
                # compute the icl prefix only once to reduce the evaluating time
                outputs, pkv = forward(args, tokenizer, model, dataset, model_batch, no_model_batch, dataset.get_icl_batch(), device, share_pkv=True, pkv=pkv)
            else:
                outputs = forward(args, tokenizer, model, dataset, model_batch, no_model_batch, icl_batch, device, share_pkv=False)
