...
from data_utils.icl_unsup_train_datasets_fast import ICLUnsupTrainDataset as ICLUnsupTrainDatasetFast
...
def prepare_dataset(args, tokenizer, rank, world_size):
    ...
    if args.unsup_fast:
        data_cls = ICLUnsupTrainDatasetFast
    else:
        data_cls = ICLUnsupTrainDataset
    
    if args.do_train:
        if args.pretrain_type in ["mixed", "icl"]:
            data["train_icl"] = data_cls(args, tokenizer, args.data_dir, args.lm_data_dir, "train", args.train_num, args.train_lm_num, args.train_ratio, args.shot, "icl", rng_sample)
            data["dev_icl"] = data_cls(args, tokenizer, args.data_dir, args.lm_data_dir, "valid", args.dev_num, args.dev_lm_num, args.dev_ratio, args.shot, "icl", rng_sample)

        if args.pretrain_type in ["mixed", "lm"]:
            data["train_lm"] = data_cls(args, tokenizer, args.data_dir, args.lm_data_dir, "train", args.train_num, args.train_lm_num, args.train_ratio, args.shot, "lm", rng_sample)
            data["dev_lm"] = data_cls(args, tokenizer, args.data_dir, args.lm_data_dir, "valid", args.dev_num, args.dev_lm_num, args.dev_ratio, args.shot, "lm", rng_sample)
        ...
        if args.pretrain_type in ["mixed", "icl"]:
            data["test_icl"] = data_cls(args, tokenizer, args.data_dir, args.lm_data_dir, "valid", args.dev_num, args.dev_lm_num, args.dev_ratio, args.shot, "icl", rng_sample)
        if args.pretrain_type in ["mixed", "lm"]:
            data["test_lm"] = data_cls(args, tokenizer, args.data_dir, args.lm_data_dir, "valid", args.dev_num, args.dev_lm_num, args.dev_ratio, args.shot, "lm", rng_sample)
    else:
...
def pretrain(args, tokenizer: GPT2Tokenizer, model: deepspeed.DeepSpeedEngine, optimizer: Adam, lr_scheduler, dataset, device):
    ...
    lm_ratio_scheduler = LMRatioScheduler(args.lm_ratio, args.end_lm_ratio, len(train_dataloader))
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch) in enumerate(train_dataloader):
            ...
            if args.eval_interval and global_step % args.eval_interval == 0 and step % args.gradient_accumulation_steps == 0:
                ...
                if args.pretrain_type in ["mixed", "lm"]:
                    dev_loss = evaluate(args, tokenizer, model, dataset["dev_icl"], "dev", epoch, "lm", device)
