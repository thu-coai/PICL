...
def prepare_dataset(args, tokenizer, rank, world_size):
    ...
    if args.do_train:
        data["train"] = ICLUnsupTrainDataset(args, tokenizer, args.data_dir, "train", args.train_num, args.train_ratio, args.shot, rng_sample)
        data["dev"] = ICLUnsupTrainDataset(args, tokenizer, args.data_dir, "valid", args.dev_num, args.dev_ratio, args.shot, rng_sample)
        ...
        data["test"] = ICLUnsupTrainDataset(args, tokenizer, args.data_dir, "valid", args.dev_num, args.dev_ratio, args.shot, rng_sample)
    else:
        raise ValueError("Do train and do eval must set one")
    return data


def forward_in_model(args, tokenizer, model: deepspeed.DeepSpeedEngine, dataset: ICLUnsupTrainDataset, model_batch, no_model_batch, icl_batch, inner_inputs, device, share_pkv=False, pkv=None):
    # if dist.get_rank() == 0:
...
def pretrain(args, tokenizer: GPT2Tokenizer, model: deepspeed.DeepSpeedEngine, optimizer: Adam, lr_scheduler, dataset, device):
    ...
    train_dataloader = DataLoader(
        dataset['train'], sampler=sampler, batch_size=args.batch_size, num_workers=args.num_workers, pin_memory=True, collate_fn=dataset["train"].collate_many)
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, icl_batch, inner_inputs) in enumerate(train_dataloader):
            ...
            outputs = forward_in_model(args, tokenizer, model, dataset["train"], model_batch, no_model_batch, icl_batch, inner_inputs, device)
            ...
            if args.eval_interval and global_step % args.eval_interval == 0 and step % args.gradient_accumulation_steps == 0:
                dev_loss = evaluate(args, tokenizer, model, dataset["dev"], "dev", epoch, device)
                model.train()
...
def evaluate(args, tokenizer, model, dataset: ICLUnsupTrainDataset, split, epoch, device):
    sampler = DistributedSampler(dataset, shuffle=False, drop_last=False)
    dataloader = DataLoader(
        dataset, sampler=sampler, batch_size=args.eval_batch_size, num_workers=args.num_workers, collate_fn=dataset.collate_many)
    ...
    with torch.no_grad():
        pkv = None
        for it, (model_batch, no_model_batch, icl_batch, inner_inputs) in enumerate(tqdm(dataloader, desc=f"Evaluating", disable=(dist.get_rank() != 0))):
    ...
    log_str = f"{split} | avg_loss: {avg_loss}"
    print_rank(log_str)
    save_rank(log_str, os.path.join(args.save, "log.txt"))
...
def main():
    ...
    if args.do_eval:
        loss = evaluate(args, tokenizer, model, dataset["test"], "test", 0, device)
