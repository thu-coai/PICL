...
class ICLUnsupTrainDataset(Dataset):
    def __init__(self, args, tokenizer, path_icl, path_lm, split, num, ratio, shot, rng_sample: random.Random):
        self.args = args
        ...
        self.mode = "icl"
        self.ctx = DistributedMMapIndexedDataset(path_icl, f"{split}_{args.unsup_data_name}", get_rank(), get_world_size())
        print_rank(f"Num ICL instances: {len(self.ctx)}")
        self.lm_ctx = []
        if self.args.lm_ratio is not None:
            ...
            if int(self.args.lm_ratio * len(self.ctx)) >= len(self.lm_ctx):
                print_rank(f"Warning: the real lm ratio in {split}:", len(self.lm_ctx) / len(self.ctx))
                self.lm_data_len = len(self.lm_ctx)
            else:
                self.lm_data_len = int(self.args.lm_ratio * len(self.ctx))
    ...
    def __len__(self):
        if self.mode == "icl":
            ...
            return len(self.ctx) + self.lm_data_len
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "lm":
            ...
            if index >= len(self.ctx):
                return self._get_lm(index-len(self.ctx))
            else:
                return self._get_icl(index)
