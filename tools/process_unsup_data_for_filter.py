...
class Encoder(object): 
    ...
    def encode(self, line):
        ...
        return input_ids, line, len(line)
...
def main():
    ...
    if args.unsup_data_max_num > 0:
        num_str = numerize(args.unsup_data_max_num)
    else:
        num_str = ""
        
    args.processed_unsup_data_path = os.path.join(args.processed_unsup_data_path, ex_prefix, num_str)
    ...
    for ith in range(0, 100):
        ...
        bin_file = os.path.join(args.processed_unsup_data_path, f"eval_filter_{ith}.bin")
        idx_file = os.path.join(args.processed_unsup_data_path, f"eval_filter_{ith}.idx")
    
        binary_builder = make_builder(bin_file, impl="mmap", dtype=np.uint16)
        
        fout_reordered = open(os.path.join(args.processed_unsup_data_path, f"eval_filter_reorder_{ith}"), "w")
        ...
        for lid, (input_ids, line, bytes_processed) in enumerate(encoded_docs):
            ...
            if args.unsup_data_max_num > 0 and sid > args.unsup_data_max_num:
                break
